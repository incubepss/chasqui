...
import { DefaultSearchPlugin, DefaultJobQueuePlugin, VendureConfig, LanguageCode, Logger } from '@vendure/core';
...
import apiOptionsConfig from './config/prod/apiOptionsConfig';
import assetServerPluginConfig from './config/prod/assetServerPluginConfig';
import adminUiPluginConfig from './config/prod/adminUiPluginConfig';
import dbConnectionConfig from './config/prod/dbConnectionConfig';
import emailPluginConfig from './config/prod/emailPluginConfig';
...
Logger.info('PROD MODE · skip build admin-ui', 'VendureConfig');
Logger.info('NODE_ENV', env.NODE_ENV);
Logger.info('ENABLED_GRAPHQL_PLAYGROUND', ENABLED_GRAPHQL_PLAYGROUND ? 'true' : 'false');
...
export const config: VendureConfig = {
  ...
  plugins: [
    ...
    HardenPlugin.init({
      ...
      apiMode: ENABLED_GRAPHQL_PLAYGROUND === true ? 'dev' : 'prod',
    }),
    ImporterOdooPlugin,
  ],
};
