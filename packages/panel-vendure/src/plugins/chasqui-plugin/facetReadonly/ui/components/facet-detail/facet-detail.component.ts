  ...
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.customFields = this.customFields.filter(cf => cf.readonly !== true);
      ...
      .pipe(
        ...
        mergeMap(([facet, languageCode]) => {
          ...
          if (valuesArray && valuesArray.dirty) {
            ...
            const updatedValues = this.getUpdatedFacetValues(facet, valuesArray as FormArray, languageCode);
  ...
  protected setFormValues(facet: FacetWithValues.Fragment, languageCode: LanguageCode) {
    ...
    if (this.customFields.length) {
      ...
      for (const fieldDef of this.customFields) {
        const cfCurrentTranslation = (currentTranslation && (currentTranslation as any).customFields) || {};
        const cf = (facet as any).customFields || {};

        const key = fieldDef.name;
        const value = fieldDef.type === 'localeString' ? cfCurrentTranslation.customFields[key] : cf[key];
  ...
  private getUpdatedFacetValues(
    ...
    const dirtyValueValues = valuesFormArray.controls.filter(c => c.dirty && c.value.id).map(c => c.value);
